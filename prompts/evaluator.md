...
gold는 다음 구조여야 합니다:
{
  "schedule_status": "yes" | "no",
  "events": [...]   // yes이면 추출, no이면 []
}
...
2. **status_correct**: schedule_status가 아래 기준에 맞는가?
   ...
   - Q1. 어떤 종류인가?
     · "yes": 나를 특정한 약속·업무요청·참석안내·합의된 약속 (수신 시각 이후, 오늘 포함)
     · "no": 결제·배송·광고·홍보·인사·남의 일정·정책 안내·미수락 제안·공고·조건부 대기·거절·취소·불특정 다수 대상 행사안내
   - 타인 일정 언급("저 오늘 회의 있어서 나중에 연락드릴게요") → "no"
   - 멀티턴 합의 미달(제안만 하고 끝, 거절, 조건부 대기) → "no"
...
7. **completeness**: message에 yes 일정이 있는데 events에서 누락된 것이 있는가?
...
2. schedule_status: exact match (2-way)
...
- **status_match**: schedule_status가 동일한가? (yes/no 2-way exact)
...
```
final_score = 0.30 * schedule_status_f1   // 2-way F1 (yes/no)
