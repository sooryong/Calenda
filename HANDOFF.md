...
## 0-NEW. ★★ r35 — criterion 기반 리베이스 1000셋 (2026-06-16)
...
- ✅ **생성**(`build_r35_negatives.py`, Haiku+criterion): 멀티턴 미확정 음성 180 → audit 이중검증 **180/180 음성확인**. (양성은 재사용, generator.md는 옛 스키마라 양성 생성 불가 → 후속 갱신 필요)
- ✅ **조립**(`build_r35_assemble.py`) → **train.jsonl 1000**: A 단일 600(양300/음300) + B 멀티 400(양183/음217), **51.7% neg**. ★음성 날짜·시각 23→**66%**, **P(양성|날짜+시각) 0.74→0.57**(base 0.48 근접 = 지름길 약화). 옛 train은 `.pre_r35bak`, 전체 정리 풀은 `_audited.jsonl`(확장용).
- ⏳ **다음: config r35(epochs=3) → push → Kaggle 학습 → golden 평가.** ★관전: ① 실카톡 음성(카카오페이·스타벅스·KB·헬로오토) 과발화 죽나(핵심) ② recall(양성 적어진 골든 19개라 측정력 약함 — 미탐 늘면 음성과다) ③ 멀티턴 확정/미확정 구분.
...
- ⚠️ **학습 전 반드시 `git push origin main`** — 노트북이 `git reset --hard origin/main`으로 clone하므로, 푸시 안 하면 이전 라운드 데이터로 돈다(r15에서 겪음). [[feedback_push_before_cloud_training]]
  - ✅ **클론 셀(1.2)이 자동 표시**(r33~): `HEAD` + `★ 이번 라운드 : {run_name}` + `train.jsonl 내 {rNN}_ 하드케이스 N건`(**0이면 push 누락** = 이전 라운드 데이터). 라운드 토큰은 `run_name`에서 파생 → 다음 라운드도 config만 바꾸면 자동.
  - ✅ **HF_TOKEN(라운드 설정 셀)**: 모델 다운로드 **전** Kaggle Secret `HF_TOKEN`을 env로 export(`HF_TOKEN`/`HUGGING_FACE_HUB_TOKEN`) → 익명 rate-limit 경고 제거·인증 다운로드. Secret 없으면 익명 폴백(공개 모델이라 동작). [[hf-adapter-backup-workflow]]
...
- ⚠️ **`num_train_epochs`는 3 고정.** r34에서 2로 줄였다가 **언더핏 회귀**(final 0.944→0.861, **동일 양성**에서 recall 0.96→0.89·time 0.92→0.78) → 3 복구. `load_best_model_at_end`가 best epoch을 고르므로 3이 과적합 위험 없음. 시간 줄이려면 epoch 말고 **데이터 축소(중복 양성 prune)** 로.
