...
## 2. 당장 할 일 — r19 배포됨, **on-device 검증** + 잔여

**즉시 검증(폰)**:
1. **앱 한 번 열기** — force-stop 후 stopped 상태라 안 열면 수집 죽음. [[feedback_reopen_app_after_gguf_swap]]
2. 설정에 **"R19 Qwen0.5B"** 표시 확인.
3. **동기회 그룹 메시지 재현**: 번호목록이 늘어나는 카톡을 보내 → 카드가 **하나로 병합 + 참석자 전원**인지, 제목이 "동기회"인지 확인.
   - ★ 병합 1순위 키가 **방이름(`EXTRA_CONVERSATION_TITLE`)** 의존 — 카톡 그룹알림이 이 값을 주는지 **logcat으로 확인**(`room=`). 비면 baseTitle 병합으로 폴백(제목 흔들리면 미병합).
4. 실업 SMS·일반 모임 제목 충실 확인.

잔여(검증 결과 따라): 방이름 미캡처면 그룹 누적 병합 대안(첫 메시지 시각 창 + 텍스트 유사도) / 제목 안정화용 r20 데이터 / 월-일("6/16") date 토큰 resolver 지원.
