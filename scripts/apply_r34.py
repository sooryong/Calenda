"""r34 데이터 적용 — 실카톡 음성 보강 하드케이스(_r34_add.jsonl)를 train.jsonl에 dedup append.

r33 실사용 진단: 서비스알림·영업로지스틱스·인사말 과발화 + 긴행사명 시각환각.
이 스크립트는 그 위에 _r34_add(서비스/금융 알림·로지스틱스·인사말 음성 + 긴행사명 무시간 양성)를 누적한다.
append 워크플로 — assemble_train --apply 금지(SOURCES r24 stale → 회귀). [[train-jsonl-append-workflow]]

사용:
    python scripts/apply_r34.py            # dry-run(통계만)
    python scripts/apply_r34.py --apply    # train.jsonl 실제 갱신
"""
...
ADD = Path("data/processed/_r34_add.jsonl")
